import { zodResolver } from "@hookform/resolvers/zod";
import { KeyRound } from "lucide-react";
import { useEffect } from "react";
import { Controller, useForm } from "react-hook-form";

import type {
  PermissionFormValues,
  PermissionListItem,
} from "@/features/permissions/Permissions.types";
import {
  PERMISSION_FORM_LIMITS,
  permissionFormApiFieldMap,
  permissionFormApiMessageFieldMap,
  permissionFormSchema,
  toPermissionFormValues,
} from "@/features/permissions/schemas/Permissions.schema";
import { mapApiErrorToForm } from "@/shared/api/client";
import { Button, Input, Modal } from "@/shared/components/ui";

type PermissionFormModalProps = {
  isOpen: boolean;
  mode: "create" | "edit";
  initialPermission?: PermissionListItem | null;
  loading?: boolean;
  apiError?: unknown;
  onClose: () => void;
  onSubmit: (values: PermissionFormValues) => Promise<void> | void;
};

export function PermissionFormModal({
  isOpen,
  mode,
  initialPermission,
  loading = false,
  apiError,
  onClose,
  onSubmit,
}: PermissionFormModalProps) {
  const { control, handleSubmit, reset } = useForm<PermissionFormValues>({
    resolver: zodResolver(permissionFormSchema),
    mode: "onChange",
    reValidateMode: "onChange",
    defaultValues: toPermissionFormValues(null),
  });

  const apiFormError = mapApiErrorToForm(
    apiError,
    permissionFormApiFieldMap,
    permissionFormApiMessageFieldMap,
  );

  useEffect(() => {
    if (isOpen) {
      reset(toPermissionFormValues(initialPermission));
      return;
    }

    reset(toPermissionFormValues(null));
  }, [initialPermission, isOpen, reset]);

  const submitForm = async (values: PermissionFormValues) => {
    await onSubmit(values);
  };

  return (
    <Modal
      isOpen={isOpen}
      onClose={loading ? () => undefined : onClose}
      title={mode === "create" ? "Crear permiso" : "Editar permiso"}
      maxWidthClassName="max-w-lg"
    >
      <form className="space-y-5" onSubmit={handleSubmit(submitForm)}>
        <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-6">
          <div className="mb-5 flex items-center gap-3">
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-sky-200 bg-sky-50 text-sky-700">
              <KeyRound size={18} />
            </span>
            <div>
              <p className="text-base font-semibold text-slate-900">
                {mode === "create" ? "Nuevo permiso" : "Ajustar permiso"}
              </p>
              <p className="mt-1 text-sm text-slate-500">
                Define el nombre del permiso para su uso dentro del sistema.
              </p>
            </div>
          </div>

          <Controller
            name="name"
            control={control}
            render={({ field, fieldState }) => (
              <Input
                label="Nombre del permiso"
                value={field.value}
                onChange={field.onChange}
                onBlur={field.onBlur}
                leftIcon={<KeyRound size={14} />}
                placeholder="manage_matches"
                maxLength={PERMISSION_FORM_LIMITS.name}
                error={fieldState.error?.message ?? apiFormError.fieldErrors.name}
                disabled={loading}
                className="bg-slate-100"
              />
            )}
          />
        </div>

        {apiFormError.globalMessage ? (
          <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            {apiFormError.globalMessage}
          </div>
        ) : null}

        <div className="flex justify-end">
          <Button variant="secondary" type="submit" disabled={loading}>
            {loading ? "Guardando..." : mode === "create" ? "Crear" : "Guardar"}
          </Button>
        </div>
      </form>
    </Modal>
  );
}
