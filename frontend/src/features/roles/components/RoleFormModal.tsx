import { zodResolver } from "@hookform/resolvers/zod";
import { ShieldCheck } from "lucide-react";
import { useEffect } from "react";
import { Controller, useForm } from "react-hook-form";

import type { RoleFormValues, RoleListItem } from "@/features/roles/Roles.types";
import {
  ROLE_FORM_LIMITS,
  roleFormApiFieldMap,
  roleFormApiMessageFieldMap,
  roleFormSchema,
  toRoleFormValues,
} from "@/features/roles/schemas/Roles.schema";
import { mapApiErrorToForm } from "@/shared/api/client";
import { Button, Input, Modal } from "@/shared/components/ui";

type RoleFormModalProps = {
  isOpen: boolean;
  mode: "create" | "edit";
  initialRole?: RoleListItem | null;
  loading?: boolean;
  apiError?: unknown;
  onClose: () => void;
  onSubmit: (values: RoleFormValues) => Promise<void> | void;
};

export function RoleFormModal({
  isOpen,
  mode,
  initialRole,
  loading = false,
  apiError,
  onClose,
  onSubmit,
}: RoleFormModalProps) {
  const { control, handleSubmit, reset } = useForm<RoleFormValues>({
    resolver: zodResolver(roleFormSchema),
    mode: "onChange",
    reValidateMode: "onChange",
    defaultValues: toRoleFormValues(null),
  });

  const apiFormError = mapApiErrorToForm(apiError, roleFormApiFieldMap, roleFormApiMessageFieldMap);

  useEffect(() => {
    if (isOpen) {
      reset(toRoleFormValues(initialRole));
      return;
    }

    reset(toRoleFormValues(null));
  }, [initialRole, isOpen, reset]);

  const submitForm = async (values: RoleFormValues) => {
    await onSubmit(values);
  };

  return (
    <Modal
      isOpen={isOpen}
      onClose={loading ? () => undefined : onClose}
      title={mode === "create" ? "Crear rol" : "Editar rol"}
      maxWidthClassName="max-w-lg"
    >
      <form className="space-y-5" onSubmit={handleSubmit(submitForm)}>
        <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-6">
          <div className="mb-5 flex items-center gap-3">
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-orange-200 bg-orange-50 text-orange-700">
              <ShieldCheck size={18} />
            </span>
            <div>
              <p className="text-base font-semibold text-slate-900">
                {mode === "create" ? "Nuevo rol" : "Ajustar rol"}
              </p>
              <p className="mt-1 text-sm text-slate-500">
                Define el nombre del rol para su uso dentro del sistema.
              </p>
            </div>
          </div>

          <Controller
            name="name"
            control={control}
            render={({ field, fieldState }) => (
              <Input
                label="Nombre del rol"
                value={field.value}
                onChange={field.onChange}
                onBlur={field.onBlur}
                leftIcon={<ShieldCheck size={14} />}
                placeholder="analista"
                maxLength={ROLE_FORM_LIMITS.name}
                error={fieldState.error?.message ?? apiFormError.fieldErrors.name}
                disabled={loading}
                className="bg-slate-100"
              />
            )}
          />
        </div>

        {apiFormError.globalMessage ? (
          <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            {apiFormError.globalMessage}
          </div>
        ) : null}

        <div className="flex justify-end">
          <Button variant="secondary" type="submit" disabled={loading}>
            {loading ? "Guardando..." : mode === "create" ? "Crear" : "Guardar"}
          </Button>
        </div>
      </form>
    </Modal>
  );
}
