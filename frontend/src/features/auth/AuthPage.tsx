...
function AuthPanelFrame({ mode, children }: AuthPanelFrameProps) {
  return (
    <div className="min-h-screen bg-[#1b1b1b] flex items-center justify-center p-0 sm:p-0">
      {/* Se eliminó el padding y se ajustó max-w al 100% para cubrir toda la pantalla */}
      <div className="flex min-h-screen w-full max-w-[100vw] overflow-hidden border-none bg-[linear-gradient(180deg,#f8fbfe_0%,#e8f0fa_52%,#dce8f6_100%)] shadow-none sm:min-h-screen">
        <HeroSection />
        {/* Panel lateral con anchos proporcionales para abarcar el resto de la pantalla */}
        <section className="relative flex w-full flex-col justify-center overflow-hidden bg-[linear-gradient(180deg,rgba(237,244,253,0.12)_0%,rgba(219,230,244,0.22)_100%)] px-8 py-9 sm:px-12 lg:w-[35%] xl:w-[30%] 2xl:w-[25%] xl:px-16">
          <div className="absolute inset-x-0 top-0 h-40 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.7),rgba(255,255,255,0)_72%)]" />
          <div className="absolute inset-x-8 top-8 z-10 sm:inset-x-12 sm:top-10">
  ...
  return (
    <section className="relative hidden flex-1 overflow-hidden lg:flex">
      <img
        ...
        alt="ScoreBlaze"
        className="absolute left-14 top-10 h-12 w-auto object-contain"
      />

      <div className="absolute inset-y-0 left-[55%] w-[180px] -translate-x-1/2 bg-[linear-gradient(180deg,rgba(175,255,178,0.44)_0%,rgba(255,244,155,0.46)_24%,rgba(255,193,139,0.44)_52%,rgba(173,212,255,0.36)_100%)] blur-[46px]" />
      <div className="absolute inset-y-0 left-[55%] w-[80px] -translate-x-1/2 bg-[linear-gradient(180deg,rgba(149,255,173,0.76)_0%,rgba(255,243,170,0.74)_26%,rgba(255,191,133,0.74)_55%,rgba(185,220,255,0.56)_100%)] opacity-90 blur-[14px]" />
      ...
      <div className="relative z-10 h-full w-full">
        <img
          ...
          className="pointer-events-none absolute bottom-[-4%] left-[6%] h-[100%] w-auto max-w-none object-contain xl:bottom-[-6%] xl:left-[8%] xl:h-[104%]"
        />
      </div>
    </section>
  );
    ...
    <AuthPanelFrame mode="login">
      <AuthCard title="Bienvenido">
        ...
        <p className="mt-9 text-center text-[13px] text-slate-600">
          No Tienes Cuenta?{" "}
          <Link to="/register"
            className="font-semibold text-slate-950 no-underline transition hover:text-orange-600"
          >
            Registrate
          </Link>
        </p>
