...
function normalizeShirtNumber(value: string | null | undefined) {
  return value?.trim() ?? "";
}

function buildShirtNumbersSignature(playerIds: number[], shirtNumbers: Record<number, string>) {
  return playerIds
    .slice()
    .sort((left, right) => left - right)
    .map((playerId) => `${playerId}:${normalizeShirtNumber(shirtNumbers[playerId])}`)
    .join("|");
}
...
function useTeamRosterSnapshot() {
  ...
  const { teams, players, memberships, loading, error } = useTeamsData();
...
type RosterMemberCardProps = {
  ...
  shirtNumber: string;
  disabled?: boolean;
  onShirtNumberChange: (playerId: number, nextValue: string) => void;
};
...
function RosterMemberCard({
  ...
  const phone = player.phone ? String(player.phone) : "Sin telefono";

  return (
    <article className="rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-[0_14px_32px_rgba(15,23,42,0.08)]">
      <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="sm:min-w-[112px]">
          <div className="rounded-[18px] border border-orange-200 bg-[linear-gradient(135deg,#fff7ed_0%,#ffffff_65%,#fffaf5_100%)] px-2 py-2 shadow-sm">
            <p className="text-center text-[8px] font-semibold uppercase tracking-[0.08em] text-orange-600">
              No. camiseta
            </p>

            <div className="mt-1.5 flex justify-center">
              <div className="grid h-[46px] w-[46px] place-items-center rounded-2xl border border-orange-200 bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
                <input
                  value={shirtNumber}
                  ...
                  maxLength={20}
                  disabled={disabled}
                  className="h-full w-full rounded-2xl bg-transparent px-1 text-center text-[18px] font-black text-slate-900 outline-none transition placeholder:text-slate-300 focus:text-orange-600 disabled:cursor-not-allowed disabled:opacity-60"
                />
              </div>
            </div>
          </div>
...
function PlayerCard({
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={cn(
        "group relative overflow-hidden rounded-2xl border px-3 py-3 transition-all duration-200",
        isAssigned
...
export default function TeamRosterPage() {
  ...
  const { selectedTeamId, hasValidTeamId, selectedTeam, loading, error, teamCount } =
    useTeamRosterSnapshot();
  const [search, setSearch] = useState("");
  const [draftShirtNumbers, setDraftShirtNumbers] = useState<Record<number, string>>({});
  const [savingShirts, setSavingShirts] = useState(false);
  const [shirtSaveError, setShirtSaveError] = useState<string | null>(null);
  const [shirtSaveFeedback, setShirtSaveFeedback] = useState<"idle" | "saved">("idle");
  ...
  const originalShirtNumbers = useMemo(
    () => Object.fromEntries((selectedTeam?.players ?? []).map((player) => [player.id, player.shirtNumber ?? ""])),
    [selectedTeam?.players]
  );
  const originalShirtNumbersKey = useMemo(
    () => buildShirtNumbersSignature(selectedTeam?.playerIds ?? [], originalShirtNumbers),
    [originalShirtNumbers, selectedTeam?.playerIds]
  );

  useEffect(() => {
    setSearch("");
    setDraftShirtNumbers(
      Object.fromEntries((selectedTeam?.players ?? []).map((player) => [player.id, player.shirtNumber ?? ""]))
    );
    setShirtSaveError(null);
    setShirtSaveFeedback("idle");
  }, [selectedTeam?.id]);

  const draftShirtNumbersKey = useMemo(
    () => buildShirtNumbersSignature(selectedTeam?.playerIds ?? [], draftShirtNumbers),
    [draftShirtNumbers, selectedTeam?.playerIds]
  );
  const shirtNumberChangeCount = useMemo(
    () =>
      (selectedTeam?.playerIds ?? []).filter(
        (playerId) => normalizeShirtNumber(draftShirtNumbers[playerId]) !== normalizeShirtNumber(originalShirtNumbers[playerId])
      ).length,
    [draftShirtNumbers, originalShirtNumbers, selectedTeam?.playerIds]
  );
  const isShirtDirty = Boolean(selectedTeam) && draftShirtNumbersKey !== originalShirtNumbersKey;
  ...
  const rosterPlayers = useMemo(
    ...
    [rosterPlayerIds, search, selectedTeam?.players]
  );

  const handleShirtNumberChange = (playerId: number, nextValue: string) => {
    setShirtSaveError(null);
    setShirtSaveFeedback("idle");
    setDraftShirtNumbers((current) => ({
      ...current,
      [playerId]: nextValue,
    }));
  };

  const resetShirtNumbers = () => {
    setShirtSaveError(null);
    setShirtSaveFeedback("idle");
    setDraftShirtNumbers(
      Object.fromEntries((selectedTeam?.players ?? []).map((player) => [player.id, player.shirtNumber ?? ""]))
  ...
  const handleSaveShirtNumbers = async () => {
    if (!selectedTeam) {
      return;
    }
    ...
    try {
      ...
      setShirtSaveFeedback("saved");
    } catch (error) {
      setShirtSaveError(error instanceof Error ? error.message : "No se pudieron guardar las camisetas.");
    } finally {
  ...
  useEffect(() => {
    if (shirtSaveFeedback !== "saved") {
      return;
    }

    const timeoutId = window.setTimeout(() => setShirtSaveFeedback("idle"), 1600);
    return () => window.clearTimeout(timeoutId);
  }, [shirtSaveFeedback]);
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Visualiza rapidamente los jugadores actuales del equipo y ajusta sus camisetas.
                    </p>
                  </div>
                  ...
                  {rosterPlayers.length > 0 ? (
                    <div className="grid gap-3 xl:grid-cols-2">
                      {rosterPlayers.map((player) => (
                        <RosterMemberCard
                          key={`roster-preview-${player.id}`}
                          player={player}
                          shirtNumber={draftShirtNumbers[player.id] ?? ""}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                          ...
                          {savingShirts
                            ...
                            : shirtSaveFeedback === "saved" && !isShirtDirty
                              ? "Camisetas guardadas"
                              : `${shirtNumberChangeCount} camiseta(s) por guardar`}
                        </span>
                      </div>

                      <div className="flex flex-wrap justify-end gap-2">
                        <Button variant="outline" onClick={resetShirtNumbers} disabled={savingShirts || !isShirtDirty}>
                          <RotateCcw size={14} />
                          Descartar
                        </Button>
                        <Button
                          variant="primary"
                          onClick={() => void handleSaveShirtNumbers()}
                          disabled={savingShirts || !isShirtDirty}
                          className="disabled:cursor-not-allowed disabled:opacity-45 disabled:saturate-50 disabled:shadow-none"
                        >
                          {savingShirts ? <LoaderCircle size={14} className="animate-spin" /> : null}
                          {!savingShirts && shirtSaveFeedback === "saved" && !isShirtDirty ? <Check size={14} /> : null}
                          {!savingShirts && !(shirtSaveFeedback === "saved" && !isShirtDirty) ? <Save size={14} /> : null}
                          {savingShirts
                            ? "Guardando..."
                            : shirtSaveFeedback === "saved" && !isShirtDirty
                              ? "Guardado"
